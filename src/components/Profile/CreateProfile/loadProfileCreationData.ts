...
export async function loadAllergenList() {
  ...
  try {
    ...
    for (const allergenUrl of allergenUrlArray) {
      const fetchedResponse = await fetch(allergenUrl + '.ttl')

      const allergenInTurtle = await fetchedResponse.text()

      const parser = new N3.Parser()

      try {
        const store = new N3.Store(parser.parse(allergenInTurtle))

        // Create a solid dataset from parsed store
        const allergenDataset = fromRdfJsDataset(store)
